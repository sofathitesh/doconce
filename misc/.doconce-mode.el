;;; doconce.el --- Mode for viewing and editing Doconce documents.
;;; Quick hack of rst.el for reStructuredText documents.
...
(defvar rst-font-lock-keywords
  ...
  (let* ( ;; This gets big - so let's define some abbreviations; the trailing
    ...
    (list
     ...
     (list
      (concat re-imp1 "\\(__" re-ima2 "__\\)" re-ims1)
     ...
     ;; `Strong Emphasis`_
     (list
      (concat re-imp1 "\\(_" re-ima2 "_\\)" re-ims1)
      2 rst-emphasis2-face)
     ;; `Emphasis`_
     (list
      (concat re-imp1 "\\(\\*" re-ima2 "\\*\\)" re-ims1)
