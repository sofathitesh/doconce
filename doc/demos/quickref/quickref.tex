...
\begin{itemize}
 \item {\LaTeX} (format {\fontsize{10pt}{10pt}\verb!latex!})
 ...
 \item MediaWiki, a la Wikipedia and Wikibooks (format {\fontsize{10pt}{10pt}\verb!mwiki!})

 \item Creoloe wiki (format {\fontsize{10pt}{10pt}\verb!cwiki!})

 \item Epydoc (format {\fontsize{10pt}{10pt}\verb!epydoc!})

 \item Pandoc (format {\fontsize{10pt}{10pt}\verb!pandoc!}, very experimental)
\end{itemize}
...
Lines starting with {\fontsize{10pt}{10pt}\verb!#!} are treated as comments in the document
...
or lists if the formats {\fontsize{10pt}{10pt}\verb!rst!} and {\fontsize{10pt}{10pt}\verb!sphinx!} are desired.
...
and
{\fontsize{10pt}{10pt}\verb!!et!} ("begin/end TeX") directives, just like computer code.
For example,
\begin{Verbatim}[fontsize=\fontsize{9pt}{9pt},tabsize=8,baselinestretch=0.85,
fontfamily=tt,xleftmargin=7mm]
\begin{align*}
...
Any {\LaTeX} syntax is accepted, but if output in the {\fontsize{10pt}{10pt}\verb!sphinx!} format
is important, one must know that {\fontsize{10pt}{10pt}\verb!sphinx!} does not support many
{\LaTeX} constructs. For output both in {\fontsize{10pt}{10pt}\verb!latex!} and {\fontsize{10pt}{10pt}\verb!sphinx!} formats we
...
\begin{itemize}
  ...
  \item Labels in multiple equation environments such as {\fontsize{10pt}{10pt}\verb!align!} are
    not (yet) handled by {\fontsize{10pt}{10pt}\verb!sphinx!}, so avoid inserting labels and referring
    to equation labels in {\fontsize{10pt}{10pt}\verb!align!} environments. Actually, {\fontsize{10pt}{10pt}\verb!align*!}
    is the preferred environment for multiple equations. Writing with
    also {\fontsize{10pt}{10pt}\verb!sphinx!} output in mind results in a slightly adjusted type
    of {\LaTeX} writing.
  ...
  \item {\LaTeX} supports lots of fancy formatting, for example, multiple
    ...
    the format (typically {\fontsize{10pt}{10pt}\verb!# #ifdef format == "latex"!}) to
    include special code for {\fontsize{10pt}{10pt}\verb!latex!} output and more straightforward
...
\begin{itemize}
  ...
  \item A name for the file with the result/answer ({\fontsize{10pt}{10pt}\verb!file=filename!}). (Optional.)

  \item A name for the solution file ({\fontsize{10pt}{10pt}\verb!solution=filename!}), where the file
...
Doconce documents may utilize a preprocessor, either {\fontsize{10pt}{10pt}\verb!preprocess!} or
